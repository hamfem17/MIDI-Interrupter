...
#define CS_LOW()		PORTB &= ~(1<<PB0);	/* Set MMC_CS = low */
#define	CS_HIGH()		PORTB |= (1<<PB0);	/* Set MMC_CS = high */
...
void start_timer(void)
{
	...
	OCR0A = ((F_CPU / 2048) / 100) - 1;
	TCCR0A = (1<<WGM01);//CTC mode
	TCCR0B = 0b101; //clock/1024
...
void power_on (void)
{
	...
	DDRB = (1<<PB1)|(1<<PB2);	//SET SCK(PB1) AS OUTPUT, MOSI(PB2) AS OUTPUT
	DDRB |= (1<<PB0);			//SET CS PIN(PD4) AS OUTPUT
	PORTB |= (1<<PB0);			//CS HIGH

	/* Enable SPI module in SPI mode 0 */
	SPCR = (1<<SPE)|(1<<MSTR); //SPI ENABLED,MASTER MODE,Frequency (Fosc/4)=4MHZ
}
